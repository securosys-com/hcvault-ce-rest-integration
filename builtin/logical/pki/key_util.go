...
func importKeyFromBytes(sc *storageContext, keyValue string, keyName string, externalKeyName string, importSecurosysHSM bool) (*keyEntry, bool, error) {
	...
	if importSecurosysHSM {
		...
		_, err = client.ImportKey(externalKeyName, string(privateKeyType), client.CleanUpHeadersAndNewLines(string(privatePemString)), client.CleanUpHeadersAndNewLines(string(publicPemString)))
		if err != nil {
			return nil, false, err
		}
