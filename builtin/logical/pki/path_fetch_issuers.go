...
func buildPathExportIssuer(b *backend, pattern string, displayAttrs *framework.DisplayAttributes) *framework.Path {
	...
	fields["pkcs11Token"] = &framework.FieldSchema{
...
func (b *backend) pathExportIssuer(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
	...
	return respondExportIssuer(issuer, pkcs11Token, key)
}
...
func respondExportIssuer(issuer *issuerEntry, pkcs11Token string, key *keyEntry) (*logical.Response, error) {
	var respManualChain []string
	for _, entity := range issuer.ManualChain {
		respManualChain = append(respManualChain, string(entity))
	}
	...
	data["privateKeyReferencePem"] = makeSecurosysKeyReference(pkcs11Token, tsbKey["keyLabel"].(string))
