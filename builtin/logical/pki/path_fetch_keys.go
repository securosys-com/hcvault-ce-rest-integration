...
func (b *backend) pathGetExportKeyHandler(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
	...
	pkcs11Token := data.Get("pkcs11Token").(string)
	...
	respData["privateKeyReferencePem"] = makeSecurosysKeyReference(pkcs11Token, tsbKey["keyLabel"].(string))
	return &logical.Response{Data: respData}, nil
}
