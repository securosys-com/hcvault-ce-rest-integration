...
ARTIFACTORY_NAME = securosys.jfrog.io/hcvault-ce-rest-integration/
...
release: prep	
		...
		for ARCH in amd64 arm64; do\
			...
			GOOS=darwin GOARCH="$${ARCH}" make bin; \
		...
		for ARCH in 386 amd64; do\
			...
			GOOS=windows GOARCH="$${ARCH}" make bin; \
		...
		for ARCH in 386 amd64 arm arm64; do\
			...
			GOOS=linux GOARCH="$${ARCH}" make bin; \
			cp bin/vault builds/vault; \
			cd builds; \
		...
		for ARCH in 386 amd64 arm; do\
			...
			GOOS=freebsd GOARCH="$${ARCH}" make bin; \
			cp bin/vault builds/vault; \
			cd builds; \
		...
		for ARCH in 386 amd64 arm; do\
			...
			GOOS=netbsd GOARCH="$${ARCH}" make bin; \
			cp bin/vault builds/vault; \
			cd builds; \
		...
		for ARCH in 386 amd64 arm; do\
			...
			GOOS=openbsd GOARCH="$${ARCH}" make bin; \
			cp bin/vault builds/vault; \
			cd builds; \
...
release-all: prep
	...
	make release VERSION=${VERSION}  RELEASE_ALL=1
...
docker: prep
	...
	GOOS=linux GOARCH="amd64" make static-dist; \
	GOOS=linux GOARCH="amd64" make bin	
    
	docker build . -t "$(IMAGE_NAME)"
	docker save "$(IMAGE_NAME)"| gzip -f > "buildDocker/$(IMAGE_NAME)_docker.tar.gz"
	cd buildDocker && shasum -a 256 "$(IMAGE_NAME)_docker.tar.gz" >> "$(IMAGE_NAME)_docker_SHA256SUM";

clean-images:
	@if [ "$$(docker images | grep 'cassandra')" != "" ]; then \
		docker rmi $$(docker images | grep 'cassandra' | awk '{ print $3}') 2> /dev/null || true ; \
	fi;
	@if [ "$$(docker images | grep 'mongodb')" != "" ]; then \
		docker rmi $$(docker images | grep 'mongodb' | awk '{ print $3}') 2> /dev/null || true ; \
	fi;
	@if [ "$$(docker images | grep 'consul')" != "" ]; then \
		docker rmi $$(docker images | grep 'consul' | awk '{ print $3}') 2> /dev/null || true ; \
	fi;
	@if [ "$$(docker images | grep 'vault_radiusd_any_client')" != "" ]; then \
		docker rmi $$(docker images | grep 'vault_radiusd_any_client' | awk '{ print $3}') 2> /dev/null || true ; \
	fi;
	@if [ "$$(docker images | grep 'vault_pki_nginx_integration')" != "" ]; then \
		docker rmi $$(docker images | grep 'vault_pki_nginx_integration' | awk '{ print $3}') 2> /dev/null || true ; \
	fi;
	@if [ "$$(docker images | grep 'vault_pki_wget_curl_integration')" != "" ]; then \
		docker rmi $$(docker images | grep 'vault_pki_wget_curl_integration' | awk '{ print $3}') 2> /dev/null || true ; \
	fi;
	@if [ "$$(docker images | grep 'vault_pki_wget_curl')" != "" ]; then \
		docker rmi $$(docker images | grep 'vault_pki_wget_curl' | awk '{ print $3}') 2> /dev/null || true ; \
	fi;
	@if [ "$$(docker images | grep 'mcr.microsoft.com/azure-storage/azurite')" != "" ]; then \
		docker rmi $$(docker images | grep 'mcr.microsoft.com/azure-storage/azurite' | awk '{ print $3}') 2> /dev/null || true ; \
	fi;
	@if [ "$$(docker images | grep 'bitnami/cassandra')" != "" ]; then \
		docker rmi $$(docker images | grep 'bitnami/cassandra' | awk '{ print $3}') 2> /dev/null || true ; \
	fi;
	@if [ "$$(docker images | grep 'gcr.io/etcd-development/etcd')" != "" ]; then \
		docker rmi $$(docker images | grep 'gcr.io/etcd-development/etcd' | awk '{ print $3}') 2> /dev/null || true ; \
	fi;
	@if [ "$$(docker images | grep 'mcr.microsoft.com/mssql/server')" != "" ]; then \
		docker rmi $$(docker images | grep 'mcr.microsoft.com/mssql/server' | awk '{ print $3}') 2> /dev/null || true ; \
	fi;
	@if [ "$$(docker images | grep 'pki_zlint')" != "" ]; then \
		docker rmi $$(docker images | grep 'pki_zlint' | awk '{ print $3}') 2> /dev/null || true ; \
	fi;
	@if [ "$$(docker images | grep 'vault_pki_zlint_validator')" != "" ]; then \
		docker rmi $$(docker images | grep 'vault_pki_zlint_validator' | awk '{ print $3}') 2> /dev/null || true ; \
	fi;
	@if [ "$$(docker images | grep 'vault-plugin-database-postgresql')" != "" ]; then \
		docker rmi $$(docker images | grep 'vault-plugin-database-postgresql' | awk '{ print $3}') 2> /dev/null || true ; \
	fi;
	@if [ "$$(docker images | grep 'vault-plugin-auth-approle')" != "" ]; then \
		docker rmi $$(docker images | grep 'vault-plugin-auth-approle' | awk '{ print $3}') 2> /dev/null || true ; \
	fi;
	@if [ "$$(docker images | grep 'multani/nomad')" != "" ]; then \
		docker rmi $$(docker images | grep 'multani/nomad' | awk '{ print $3}') 2> /dev/null || true ; \
	fi;
		@if [ "$$(docker images | grep 'docker.mirror.hashicorp.services')" != "" ]; then \
		docker rmi $$(docker images | grep 'docker.mirror.hashicorp.services' | awk '{ print $3}') 2> /dev/null || true ; \
	fi;
	@if [ "$$(docker images | grep 'securosys.jfrog.io/hcvault-ce-rest-integration')" != "" ]; then \
		docker rmi $$(docker images | grep 'securosys.jfrog.io/hcvault-ce-rest-integration' | awk '{ print $3}') 2> /dev/null || true ; \
	fi;
	@if [ "$$(docker images | grep 'securosys.jfrog.io/internal-hcvault-ce-rest-integration')" != "" ]; then \
		docker rmi $$(docker images | grep 'securosys.jfrog.io/internal-hcvault-ce-rest-integration' | awk '{ print $3}') 2> /dev/null || true ; \
	fi;
	@if [ "$$(docker images | grep 'mcr.microsoft.com/azure-storage/azurite')" != "" ]; then \
		docker rmi $$(docker images | grep 'mcr.microsoft.com/azure-storage/azurite' | awk '{ print $3}') 2> /dev/null || true ; \
	fi;
