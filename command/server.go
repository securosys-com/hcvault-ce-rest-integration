...
func (c *ServerCommand) runRecoveryMode() int {
	...
	hsm.RunTimeInfo["log level"] = config.LogLevel
	...
	hsm.RunTimeInfo["storage"] = config.Storage.Type

	if coreConfig.ClusterAddr != "" {
		info["cluster address"] = coreConfig.ClusterAddr
		hsm.RunTimeInfo["cluster address"] = coreConfig.ClusterAddr
	...
	hsm.RunTimeInfo["version"] = verInfo.FullVersionNumber(false)

	if verInfo.Revision != "" {
		info["version sha"] = strings.Trim(verInfo.Revision, "'")
		infoKeys = append(infoKeys, "version sha")
		hsm.RunTimeInfo["version sha"] = strings.Trim(verInfo.Revision, "'")
	}

	infoKeys = append(infoKeys, "recovery mode")
	info["recovery mode"] = "true"
	hsm.RunTimeInfo["recovery mode"] = "true"

	infoKeys = append(infoKeys, "go version")
	info["go version"] = runtime.Version()
	hsm.RunTimeInfo["go version"] = runtime.Version()

	fipsStatus := getFIPSInfoKey()
	if fipsStatus != "" {
		infoKeys = append(infoKeys, "fips")
		info["fips"] = fipsStatus
		hsm.RunTimeInfo["fips"] = fipsStatus
	}
...
func (c *ServerCommand) Run(args []string) int {
	...
	hsm.RunTimeInfo["type"] = "production"
	if c.flagDev {
		hsm.RunTimeInfo["type"] = "development"
	}
	...
	hsm.RunTimeInfo["mlock"] = fmt.Sprintf(
		"supported: %v, enabled: %v",
		mlock.Supported(), !config.DisableMlock && mlock.Supported())
	if coreConfig.ClusterAddr != "" {
		info["cluster address"] = coreConfig.ClusterAddr
	...
	if coreConfig.RedirectAddr != "" {
		...
		hsm.RunTimeInfo["api address"] = coreConfig.RedirectAddr
		infoKeys = append(infoKeys, "api address")
	}

	if config.HAStorage != nil {
		info["HA storage"] = config.HAStorage.Type
		hsm.RunTimeInfo["HA storage"] = config.HAStorage.Type
		...
		if coreConfig.HAPhysical != nil {
			if coreConfig.HAPhysical.HAEnabled() {
				...
				hsm.RunTimeInfo["storage"] += " (HA available)"
			} else {
				info["storage"] += " (HA disabled)"
				hsm.RunTimeInfo["storage"] += " (HA disabled)"
			}
		}
	}
	...
	info["cgo"] = "disabled"
	hsm.RunTimeInfo["cgo"] = "disabled"
	if version.CgoEnabled {
		info["cgo"] = "enabled"
		hsm.RunTimeInfo["cgo"] = "enabled"
	}

	infoKeys = append(infoKeys, "recovery mode")
	info["recovery mode"] = "false"
	hsm.RunTimeInfo["recovery mode"] = "false"
...
func (c *ServerCommand) enableThreeNodeDevCluster(base *vault.CoreConfig, info map[string]string, infoKeys []string, devListenAddress, tempDir string) int {
	...
	hsm.RunTimeInfo["cluster parameters path"] = testCluster.TempDir
	...
	for i, core := range testCluster.Cores {
		...
		hsm.RunTimeInfo[fmt.Sprintf("node %d api address", i)] = fmt.Sprintf("https://%s", core.Listeners[0].Address.String())
